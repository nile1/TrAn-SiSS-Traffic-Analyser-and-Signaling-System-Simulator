...
int graph_add_street(struct graph *g,int tsj1_id,int tsj2_id , char st_name , int type)
{
	...
	else
	{
    	...
    	street_t *new_street=create_street(st_name,type,MAX_VEHICLES,timer ) // timer to be taken from stimulation function//	
		
		newlane_id=g->num_of_streets;
		
		/*Allocate Memory to the Edge*/
    	g->street_list[g->num_street-1]=new_lane;
...
int graph_delete_street(struct graph *g, int tsj1_id, int tsj2_id)
{
	...
	st = g->street_list[edge_id];
	
	/*Get TSJ id's the street is connecting*/
    t1 = get_tsj_ptr(g,tsj1_id);
	t2 = get_tsj_ptr(g,tsj2_id);
    
	/*Set the connection from TSJ 1 to TSJ 2 and TSJ2 and TSJ 1 to zero(0)*/
    
	
	/*Deallocate the string that is deleted*/
