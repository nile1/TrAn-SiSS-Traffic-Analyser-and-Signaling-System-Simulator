...
map_extn_t *create_map_extn(char *name, int rate_of_generation , traffic_sig_jn_t *adj_tsj , street_t *connecting_street );
map_extn_list_t *create_map_extension_list(void);
int add_map_extn(char *name, int rate_of_generation, traffic_sig_jn_t *adj_tsj, street_t *connecting_street, map_extn_list_t *map_extn_head );
...
int graph_delete_street(struct graph_tsj *g, int edge_id);
void simulate(graph_tsj_t *g,map_extn_t *m);
...
void tsj_rename(graph_tsj_t *g,char *id,char *name);
...
int graph_delete_tsj(graph_tsj_t *g,traffic_sig_jn_t *t)
{
	...
	else
	{
		...
		while(curr!=NULL)
		{
			...
			temp=curr;
			graph_delete_street(g , get_id(curr->dest_tsj_p) );
			curr=curr->next;
			free(temp);
		}
	}
