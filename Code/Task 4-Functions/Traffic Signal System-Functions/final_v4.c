...
street_t *get_street_pointer(traffic_sig_tsj *tsj, street_t *s, int k);
...
street_t *get_street_pointer(traffic_sig_tsj_t *tsj, street_t *s, int k) // function to return required street pointer either adjacent or opposite
{
    int i,j;
    street_t *temp[4];
...
void set_vehicle(vehicle_t *vehicle, graph_tsj_t *g, map_extn_list_t *extn_list, map_extn_t *ptr )
{
    ...
    vehicle->speed = random()%100;
    strcpy(vehicle->type,random_vehicle());
    vehicle->source = ptr;

    do
    {
        num_extn = random()%extn_list->num_map_extn;
        temp = ptr->head;
